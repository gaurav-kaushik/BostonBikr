# -*- coding: utf-8 -*-
"""
Created on Wed Jul 15 10:29:18 2015
...
class Graph(Vertex):
    ...
    def addEdges(self, f, t):
        # let's input a structure to get our edges to plot
        # to plot, each instances needs to be a list of coordinates
        # we're starting with a LIST of LISTS but could do DICT of LISTS
        # if this takes fucking forever.
        self.edges.append([list(f), list(t)])
    ...
    def ccBFS(self, startN, visited, conComponent):
        ...
        #remove isolated islands -- but HOW does it remove isolated island?
        ###
        visited.add(startN)
...
def createMiniWorld(miniGraph, startPt, endPt): 
    ...
    cursor.execute("SELECT * FROM Edges WHERE fnodelng>{} and fnodelat>{} \
                 and fnodelng<{} and fnodelat<{} \
                ".format(minLng, minLat, maxLng, maxLat))
    miniDB = cursor.fetchall()
    #for data in miniDB:
    #    print data
...
def PathTestMashUp(startPt, endPt, runDis):
    ...
    startCor = google2coords(startPt)
    endCor = google2coords(endPt)
